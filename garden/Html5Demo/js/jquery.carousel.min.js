(function(c) {
	function ga(z, s) {
		function m() {
			...
			o(g);
//			!c.support.leadingWhitespace  || N(0);
			!c.support.leadingWhitespace || N(0);
...
//			c.browser.msie && N(b)
			!c.support.leadingWhitespace && N(b)
		}
		...
		function N(b) {
			...
			c("<img/>").load(function() {
				...
				o(g);
				!c.support.leadingWhitespace  || b < u - 1 && N(++b)
			}).attr("src", d)
		}
		...
		(function() {
			...
			if (a.settings.xmlSource) {
				...
				c.ajax({
					...
					dataType: !c.support.leadingWhitespace  ? "text" : "xml",
					success: function(b) {
						var d;
	...
	c.fn.carousel.defaults = {
		...
		width: 500,
		height: 300,
		itemWidth: 100,//每个图片的宽度
		itemHeight: 100,//每个图片的高度
