...
import {getShoppingBonusPocketList,checkBonusPocket,confirmCheckBonusPocket} from '../actions/ShoppingActions.js';
class ShoppingBonusPocketList extends Component {
    componentDidMount(){
        this.props.getShoppingBonusPocketList(0,10);
        };
    componentWillUpdate(){
        ...
        window.onscroll=(e)=>{
            if((e.target.body.scrollTop+window.innerHeight)==e.target.body.offsetHeight){
                this.props.getShoppingBonusPocketList(parseInt(this.props.bonusPocket.list.length/10),10);
            }
        };
    }
    componentDidUnMount(){
    ...
    onPocketCheck(key,e,checked){
        this.props.checkBonusPocket(key,checked);
    }
    onConfirmPocket(){
        this.props.confirmCheckBonusPocket();
        this.refs.ShoppingConformOrder.click();
    }
    ...
    render() {
        const {products,bonusPocket,browserVersion,productTempBuy,productCart}=this.props;
        ...
        var checkedTotal=0;
        bonusPocket.list.map(item=>item.checked==true?checkedTotal=(parseFloat(checkedTotal)+parseFloat(item.detail_fee)):"");
        return (
            <List style={styles.list}>
                ...
                <ListItem style={styles.itemProducts} disabled={true}><h3 style={styles.productCount}>可用优惠券</h3></ListItem>
                <ListDivider style={{height:"2px"}}/>
                <div style={styles.imageDiv}>
                        {bonusPocket.list.map((item,index)=>
                            ((this.props.params.type==2&&item.pids.filter(p_kind=>p_kind==productTempBuy.p_kind).length>0)
                        ||(this.props.params.type==1&&item.pids.filter(p_kind=>productCart.filter(cart=>cart.checked&&cart.p_kind==p_kind).length>0).length>0))?
                    <div key={time*2+index} style={{position:"relative",height:"100px"}}>
                        ...
                        <div style={styles.pocketBG}>
                            <div style={{float:"left",marginLeft:"10px",marginTop:"23px",width:"100px",display:"inline-block",borderRight:"2px solid #f90"}}>
                                <font style={{color:"red",fontSize:"18px"}}>￥</font><font style={{color:"red",fontSize:"30px"}}>{parseFloat(item.detail_fee).toFixed(0)}</font>
                            </div>
                            <div style={{float: "left",display: "inline-block",paddingLeft: "0.8em",paddingTop:".4em"}}>
                                <h4 style={{ lineHeight: "1.6em",fontSize: ".45em",color: "#fff",background: "#f90",textAlign: "center",borderRadius: "12px"}}>{"限:"+item.product}</h4>
                                <p style={{height:"18px"}}>
...
function mapDispatchToProps(dispatch) {
    return bindActionCreators({getShoppingBonusPocketList,checkBonusPocket,confirmCheckBonusPocket}, dispatch);
}
export default connect(mapStateToProps,mapDispatchToProps)(ShoppingBonusPocketList);