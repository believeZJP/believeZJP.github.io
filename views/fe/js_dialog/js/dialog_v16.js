		...
		renderUI: function () {
			...
			switch (this.cfg.winType){
				...
				case 'confirm':
					...
					break;
				case 'prompt':
					this.cfg.content += '<p class="dialog-prompt-inputWrapper"><input type="'+(this.cfg.isPromptInputPassword ? "password":"text")+'" value="'+this.cfg.defaultValue4PromptInput+'" maxlength="'+this.cfg.maxLength4PromptInput+'" class="dialog-prompt-input"></p>';
					footerContent = '<input type="button" value="'+ this.cfg.text4PromptBtn+'" class="dialog-prompt-btn"><input type="button" value="'+this.cfg.text4CancelBtn+'" class="dialog-cancel-btn">';
					break;
			...
			if(this.cfg.winType != 'common'){
				this.boundingBox.prepend('<div class="dialog-header">'+ this.cfg.title +'</div>');
			...
			this._promptInput = this.boundingBox.find('.dialog-prompt-input');//此处暂时将输入框存起来，以后要用到
		},
		bindUI: function () {
			var that = this;
			this.boundingBox.on('click', '.dialog-alert-btn', function () {
				...
				that.fire('cancel');
				that.destory();
			}).on('click', '.dialog-prompt-btn', function () {
				//此处需要将输入框的值传给回调
